...
docker exec trimiata-events-clickhouse clickhouse-client \
	...
	--send_timeout 20 \
	--query "
	INSERT INTO ${CLICKHOUSE_DB}.events_raw
	(
...
docker exec trimiata-events-clickhouse clickhouse-client \
	...
	--receive_timeout 20 \
	--send_timeout 20 \
	--query "SELECT event_time, event_type, product_id, user_key FROM ${CLICKHOUSE_DB}.events_raw ORDER BY ingested_at DESC LIMIT 5"
