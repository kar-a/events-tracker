...
Добавлен Node.js API-коллектор (`POST /v1/events`, `POST /v1/events/batch`) со stub-записью (следующий шаг — реальная вставка в ClickHouse).
...
После запуска Grafana доступна по адресу `http://localhost:${GRAFANA_PORT}`.
